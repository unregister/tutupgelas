$(function(){
	...
	$('span.callout-close').click(function(){
		$('.callout').slideUp();
	});
...
var alert_message = function(type_message, text_message)
{
	...
	if(type_message == 'error'){
		var class_css = 'callout callout-danger';	
	}else if(type_message == 'success'){
		var class_css = 'callout callout-success';	
	}
	
    $('.modal-message.'+type_message).remove();
    $('#message-box').prepend('<div class="modal-message '+class_css+' '+type_message+'"><span class="icon"></span><span class="callout-close close">x</span>'+text_message+'</div>');
    $('html, body').animate({
        scrollTop:0
    }, 600);
...
function goToList(title_modal, message_text){
   ...
   var modal_content = '<div id="dialog_modal_message" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="dialog_modal_message_label" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title"><i class="fa fa-exclamation-triangle" style="color:#F39C12;"></i> ' + title_modal + '</h4></div><div class="modal-body">'+ message_text + '</div><div class="modal-footer"><button type="button" class="btn btn-danger cancel-confirmation" data-dismiss="modal"><i class="fa fa-times-circle"></i> Batal</button><button type="button" class="btn btn-primary ok-confirmation" data-dismiss="modal" aria-hidden="true" id="ok"><i class="fa fa-check"></i> Ok</button></div></div></div></div>';
