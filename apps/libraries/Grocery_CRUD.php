...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->unset_delete_multiple = $this->unset_delete_multiple;
	...
	protected function get_integer_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' class='numeric form-input col-xs-8' $extra_attributes />";
		return $input;
	}
	...
	protected function get_string_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<input id='field-{$field_info->name}' class='form-input col-xs-8' name='{$field_info->name}' type='text' value=\"$value\" $extra_attributes />";
		return $input;
	}
	...
	protected function get_text_input($field_info,$value)
	{
		if($field_info->extras == 'text_editor')
		{
			...
			$input = "<textarea id='field-{$field_info->name}' name='{$field_info->name}' class='form-input $class_name' >$value</textarea>";
		}
		else
		{
			$input = "<textarea id='field-{$field_info->name}' name='{$field_info->name}' class='form-input col-xs-8'>$value</textarea>";
		}
		return $input;
	}
	...
	protected function get_datetime_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$datetime' maxlength='19' class='datetime-input form-input' />
	...
	protected function get_password_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' class='form-input col-xs-8' type='password' value='$value' $extra_attributes />";
		return $input;
	}
	...
	protected function get_date_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$date' maxlength='10' class='datepicker-input form-input' style='width:100px' />
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select form-input' data-placeholder='".$select_title."'>";
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}'  name='{$field_info->name}' class='$ajax_or_not_class form-input' data-placeholder='$select_title' style='width:200px'>";
	...
	protected function get_upload_file_readonly_input($field_info,$value)
	{
		...
		if( in_array($ext,array('jpg','jpeg','png','gif','bmp'))){
			$value = '<img src="'.$file.'" style="width:100px" />';
		}
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input .= $is_image ? " $image_class'><img src='".$file_url."' height='100px'>" : "' target='_blank'>$value";
		$input .= "</a> ";
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $unset_delete_multiple= false;
	...
	/**
	 * Unsets the delete multiple operation from the list
	 *
	 * @return	void
	 */
	
	public function unset_delete_multiple()
	{
		$this->unset_delete_multiple = true;
